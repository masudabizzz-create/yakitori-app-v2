...
-- --------------------
CREATE TABLE IF NOT EXISTS daily_log_edits (
...
CREATE INDEX IF NOT EXISTS idx_daily_log_edits_tenant ON daily_log_edits(tenant_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_daily_log_edits_log ON daily_log_edits(daily_log_id, created_at DESC);
...
-- 店舗責任者以上: 履歴作成可能
DROP POLICY IF EXISTS "daily_log_edits_insert" ON daily_log_edits;
